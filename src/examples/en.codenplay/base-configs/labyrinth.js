...
function executingError(context, msg) {
  context.field.log(`Error: ${msg}. Fix the code and try again ;)`, { error: true });
  throw new Error(msg);
}
    ...
    return {
      ...
      async post(context) {
        ...
        if (!taskFinished(path, context.state.path)) {
          executingError(context, 'You need to get through the labyrinth');
        }
        // Clear state
        context.state.path = []
      },
...
function prepareLabyrinth({ iterations, stepsArgumentSupported, startCodeVal, solutionCode, codeFontSize }) {  
  let conf = {
    ...
    taskDescription: 'Write directions for the caveman to get through the labyrinth',
    codeFontSize,
    startCodeVal,
    solutionCode,
    tickHooks: {
      ...
      async run(context, params) {
        function validateParams(params) {
          ...
          if (params && params.length == 1 && !stepsArgumentSupported) {
            throw new Error(`This instruction doesn't accept arguments`);
          }
        }
        ...
        context.field.log(`The caveman is making ${stepsAmount} steps in direction: ${direction}...`);
        ...
        async function runStep() {
          ...
          if (lastStep.direction !== context.state.correctPath[len - 1].direction
            ...
            executingError(context, `Hmm, you can't go there`);
          }
          if (len > 1) {
  ...
  let docs = {
    right: {
      ...
      text: `The caveman goes right. Instruction doesn't accept arguments`,
    },
    left: {
      examples: 'man.moveLeft();',
      text: `The caveman goes left. Instruction doesn't accept arguments`,
    },
    up: {
      examples: 'man.moveUp();',
      text: `The caveman goes up. Instruction doesn't accept arguments`,
    },
    down: {
      examples: 'man.moveDown();',
      text: `The caveman goes down. Instruction doesn't accept arguments`,
    },
  };
  ...
  if (stepsArgumentSupported) {
    docs = {
      right: {
        ...
        text: `The caveman goes right. Instruction accept single argument: number of steps`,
      },
      left: {
        examples: 'man.moveLeft(1); <br /> man.moveLeft(2);',
        text: `The caveman goes left. Instruction accept single argument: number of steps`,
      },
      up: {
        examples: 'man.moveUp(1); <br /> man.moveUp(4);',
        text: `The caveman goes up. Instruction accept single argument: number of steps`,
      },
      down: {
        examples: 'man.moveDown(1); <br /> man.moveDown(2);',
        text: `The caveman goes down. Instruction accept single argument: number of steps`,
      },
    };
  }
