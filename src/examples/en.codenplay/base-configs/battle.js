...
function prepareBattle({iterations, startPosX, maxTicksToWin, startCodeVal, solutionCode, stepsArgSupported, codeFontSize, shortDescription}) {
      ...
      iterationsConf.push({
        ...
        async post({state, field}) {
          if (!allEnemiesKilled(state)) {
            runtimeError('Not all enemies were defeated', field);
          }
        }
      });
    }
  ...
  async function animateDeath(id, field) {
    ...
    if (id == 'Hero') {
      ...
      field.log(`Unfortunately, the hero was killed. Please try again!`);
    } else {
  ...
  let conf = {
    ...
    taskDescription: `Just kill your enemies and don't let them kill you. The game has rounds. You should write instructions for the hero for each round.`,
    size: {
      width: 550,
      height: 450
    },
    ...
    methods: {
      'hero.go': {
        doc: (stepsArgSupported ? 'The hero goes right. Instruction accepts single argument: number of steps.' : `The hero goes right. Instruction doesn't accept arguments.`),
        ...
        async run({field, state}, params) {
          ...
          if (params.length > 0) {
            if (stepsArgSupported) {
              ...
              runtimeError('Instruction hero.go does not accept arguments', field);
            }
          }

          field.log(`The hero is making ${stepsNumber} steps...`);
      ...
      'hero.swordAttack': {
        doc: `Sword attack. This instruction doesn't accept arguments.`,
      ...
      'hero.spearAttack': {
        doc: `Spear attack. This instruction doesn't accept arguments.`,
      ...
      'hero.attackWith': {
        ...
        doc: `Attack instruction. It accepts single String argument: weapon (it can be 'spear' or 'sword')`,
        examples: 'hero.attackWith(\'spear\'); <br /> hero.attackWith(\'sword\');',
        async run({field, state}, args) {
          if (args.length != 1) {
            runtimeError('hero.attackWith accept exactly one argument: \'spear\' or \'sword\'', field);
          }
          let arg = args[0];
          if (arg == 'spear') {
            await spearAttack({field, state});
      ...
      'hero.defend': {
        doc: `Instruction to defend using shield. This instruction doesn't accept arguments.`,
        examples: 'hero.defend();',
        async run({field, state}) {
          field.log('The hero is defending...');
      ...
      'hero.skip': {
        doc: `Use it to do nothing in current round. This instruction doesn't accept arguments.`,
        examples: 'hero.skip();',
        async run({field, state}) {
          field.log('The hero is skipping their move...');
    ...
    tickHooks: {
      ...
      async post(tickNr, {state, field}) {
        if (tickNr >= maxTicksToWin) {
          runtimeError(`You can not use more than ${maxTicksToWin} rounds to win`, field);
        }
        ...
        for (const enemyId in state.enemies) {
          ...
          if (enemy.action(tickNr) == 'attack') {
            ...
            field.log(`${enemyId} just waiting...`);
        ...
        if (heroKilled) {
          ...
          runtimeError('The hero was killed :(', field);
        }
      }
    },
    startCodeVal,
